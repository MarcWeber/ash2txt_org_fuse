...
type LazyFolder struct {
	...
	wg           *sync.WaitGroup
	flusher      *Flusher
	folderCache  map[string]*LazyFolder // In-memory folder cache
	fileCache    map[string]*LazyFile   // In-memory file cache
	foldersFiles sync.Once              // Ensure cache is populated once
}
...
func NewLazyFolder(path, cacheDir, rootURL string) *LazyFolder {
	...
	f := &LazyFolder{
		...
		folderCache: make(map[string]*LazyFolder),
		fileCache:   make(map[string]*LazyFile),
	}
	return f
}
...
func (f *LazyFolder) FoldersAndFiles() (map[string]*LazyFolder, map[string]*LazyFile, error) {
	f.foldersFiles.Do(func() {
		...
		for _, k := range c.Folders {
			...
			f.folderCache[k] = subFolder
		}
		for k := range c.Files {
			f.fileCache[k] = &LazyFile{name: k, parent: f}
		}
	})
	if len(f.folderCache) == 0 && len(f.fileCache) == 0 {
		_, err := f.Cached() // Ensure cache is populated if not already
		if err != nil {
			return nil, nil, err
		}
	}
	return f.folderCache, f.fileCache, nil
}
...
func (f *LazyFolder) DuApproximate(path string) error {
	...
	total, err := folder.duApproximate()
	if err != nil {
		return err
	}
	fmt.Printf("%d %s\n", total, folder.path)
...
func (f *LazyFolder) duApproximate() (int64, error) {
	...
	for _, file := range files {
		size, err := file.SizeBytesApproximate()
		if err != nil {
			return 0, err
		}
		total += size
...
// Exported methods for FUSE compatibility
func (f *LazyFolder) Path() string {
	return f.path
}
